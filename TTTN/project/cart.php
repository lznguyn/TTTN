...
session_start(); // Bắt đầu làm việc với session
$user_id = $_SESSION['user_id']; // Lấy id người dùng

if (!isset($user_id)) { // Kiểm tra nếu chưa có id chuyển đến trang login
    header('Location: login.php');
}

if (isset($_POST['update_cart'])) { // Kiểm tra form có yêu cầu cập nhật giỏ hàng
...
if (isset($_GET['delete'])) { // Xóa một sản phẩm
    $delete_id = $_GET['delete'];
    mysqli_query($conn, "DELETE FROM `cart` WHERE id = '$delete_id'") or die('query failed');
    header('location:cart.php');
}

if (isset($_GET['delete_all'])) { // Xóa tất cả sản phẩm
                ...
                while ($fetch_cart = mysqli_fetch_assoc($select_cart)) {
                    ...
                    <div class="box">
                        ...
                        <div class="sub-total">Tổng: 
                            <span><?php echo number_format($sub_total = $fetch_cart['quantity'] * $fetch_cart['price'], 0, ',', '.'); ?> VNĐ</span>
                        </div>
                    </div>
                    <?php
