...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($name !== '' && $value !== '') {
            $stmt = $conn->prepare(
                "INSERT INTO categories (name, value, sort_order, is_active)
                 VALUES (?, ?, ?, ?)"
            );
...
$categories = [];
if ($result) {
    $categories = $result->fetch_all(MYSQLI_ASSOC); // mysqli_result::fetch_all
    $result->free();
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Admin - Cấu hình danh mục laptop</title>
...
<div class="max-w-4xl mx-auto py-10">
    <h1 class="text-2xl font-bold mb-6">Cấu hình danh mục laptop</h1>
